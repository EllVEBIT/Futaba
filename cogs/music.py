...
class Song:
  ...
  def create_embed(self):
    embed = (discord.Embed(title='Сейчас играет',description='```css\n{0.source.title}\n```'.format(self),color=discord.Color.dark_red()).add_field(name='Продолжительность', value=self.source.duration).add_field(name='Запрос от', value=self.requester.mention).add_field(name='Загрузчик', value='[{0.source.uploader}]({0.source.uploader_url})'.format(self)).add_field(name='URL', value='[Click]({0.source.url})'.format(self)).set_thumbnail(url=self.source.thumbnail))
...
class Music(commands.Cog):
  ...
  async def _leave(self, ctx: commands.Context):
    if not ctx.voice_state.voice:
      return await ctx.send('Не подключен ни к одному из голосовых каналов.')
